...
-(void)allUserReq{
    ...
    [api requestWithObject:@[@(latestUpdateTime)] Completion:^(id response, NSError *error) {
        if (!error) {
            ...
            NSInteger responseLastUpdateTime = [[(NSDictionary*)response objectForKey:@"latestUpdateTime"] integerValue];
            
            if(latestUpdateTime==0|| latestUpdateTime!=responseLastUpdateTime){
                NSMutableArray *userArray = [(NSDictionary*)response objectForKey:@"userlist"];
